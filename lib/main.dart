...
class Channel {
  ...
  Channel({
    ...
    this.delay = "-",
    this.resolution = "-",
    this.isSelected = true, // 默认导入即勾选，方便测速
  });
}
...
class _IPTVTesterHomeState extends State<IPTVTesterHome> {
  ...
  Future<void> _startBatchDownload() async {
    ...
    _addLog("批量下载导入结束。成功: $success，失败: ${urls.length - success}");
  }
  ...
  Future<void> _startTest() async {
    ...
    const concurrency = 25; // 提高测速并发到 25 线程
  ...
  Future<void> _testSingleChannel(Channel ch) async {
    ...
    try {
      ...
      request.headers.set("Range", "bytes=0-1023"); // 1KB Range截断限制
  ...
  void _copyAllLogs() {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text("已复制所有调试日志"), duration: Duration(seconds: 1)),
    );
  }
